...
public class MEjSingleton {
            ...
            try {
                ...
                switch (opcion) {
                    ...
                    case 4:
                        ...
                        if (!existeUsuarioNombreEmial(nombreUsuario, emailUsuario)) {
                            crearUsuario(nombreUsuario, emailUsuario, anho_nacimiento);
                        } else {
                            System.out.println("Error: El Usuario con ese nombre, email ya existe.");
                        }
                        break;
                    case 5:
    ...
    // Método para verificar si el proveedor ya existe en la base de datos
    static boolean existeUsuarioNombreEmial(String nombre, String email) {
        String sql = "SELECT COUNT(*) FROM usuarios WHERE nombre = ? OR email = ?";
        ...
        try (PreparedStatement stm = mysqlconn.prepareStatement(sql)){
            ...
            System.out.println("Error al verificar la existencia del usuario con ese nombre, email: " + e.getMessage());
        }
        return false;
    }
