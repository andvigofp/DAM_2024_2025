...
delimiter //
 create procedure get_libro_by_id(IN id int)
 begin 
	select * from libros where codigo = id;
  end //
 delimiter ;
 
 delimiter //
 create procedure contar_libros_precio(IN precio decimal(5,2), OUT total int)
 begin 
	select count(*) into total from libros l where l.precio >= precio;
  end //
 delimiter ;